...
>
    ...
    },
    {
        ...
        "molFilter": true,
        "molString": "it.unibo.collektive.examples.maxId.MaxIdKt.maxID",
        "molPropertyFilter": true,
        "property": "",
...
fun Aggregate<Int>.maxNetworkID(environment: EnvironmentVariables): Int {
    ...
    val maxValue = neighboring(maxLocalValue).max(maxLocalValue)
    ...
    val subnetDiameter = subnetDiameter(environment["maxNetworkID"], environment["distanceToSource"])

    environment["subnetDiameter"] = subnetDiameter

    val subnetDiameterDistance = subnetDiameter.distance

    environment["subnetDiameterDistance"] = subnetDiameterDistance
